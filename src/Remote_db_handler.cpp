...
protected:
	...
	DB_cache dbCache;								// Database connection
...
public:
	...
	/**************************************************************************************************/
	Redis_handler(const std::string& _host, const int& _port, const std::string& _passwd, const int& _db) :
	...
	/**************************************************************************************************/
	Redis_handler() :
		Redis_handler(HOST_IP, DEFAULT_PORT, HOST_PASSWD, HOST_DB) {}
	...
	bool RTDB_RemoveAsset(const std::string &sAsset)
	{
		...
		{
			StringSet devices;
			dbCache.smembers(sAsset + sDeviceListSep, devices);		// Gets devices in the asset

			StringSet types;
			dbCache.smembers(sAsset + sTypeListSep, types);			// Gets types in the asset
	...
	TEQ_LIST_PTR RTDB_GetDeviceTypes(const std::string &sAsset)
	{
		...
		dbCache.smembers(sAsset + sTypeListSep, sSet);		// Gets the types in the asset
	...
	VAR_LIST_PTR RTDB_GetVariables(const std::string &sAsset, const std::string &sTypeOfDevice)
	{
		...
		dbCache.smembers(sAsset + sTypeVarsSep + sTypeOfDevice, sSet); 		// Obtains all variables
	...
	EQ_LIST_PTR RTDB_GetDevices(const std::string &sAsset, const std::string &sDeviceType)
	{
		...
		dbCache.smembers(sAsset + sDeviceTypeListSep + sDeviceType + sValue, sSet);
	...
	{		
		...
		dbCache.smembers(sAsset + sDeviceTypeListSep + sTypeOfDevice, oSet);	// Gets all devices of the given type
	...
	{		
		...
		{
			StringSet sSet;
			dbCache.smembers(sAsset + sDeviceTypeListSep + sTypeOfDevice, sSet); 		// Gets all devices of sTypeOfDevice type
	...
	{
		ASSET_LIST sSet;
		dbCache.smembers(sASSETS, sSet);		// Gets all asset names
